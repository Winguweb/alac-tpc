  ...
  def stream_xlsx()
      ...
      reports.each do | report|
        if characterization = Characterization.where(case_id: report[0]).first
          ...
          sheet << [
            ...
            char_data["¿Por qué?"],
            char_data["¿Pertenece usted a alguna de las siguientes poblaciones? ¿Cual?"]
          ]
        end
      end
    ...
    xlsx.write_worksheet 'Asesoría Legal' do |sheet|
      ...
      Advisory.all.each do |advisory|
        sheet << [advisory.characterization.case_id,advisory.id, advisory.tracking_date,advisory.kind_answer,advisory.summary]
      end
    end
